      ...
      return (
        <div className="bg-slate-800 border border-white/10 rounded-lg p-3 shadow-lg">
          ...
          {payload.map((entry, index) => (
            <p key={`${(entry && entry.name) ? entry.name : 'payload'}__${index}`} className="text-xs" style={{ color: entry.color }}>
    ...
    <div className="rounded-lg border border-white/10 bg-slate-900 p-4">
      <div className="mb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-6">
          ...
          <div className="bg-slate-800/50 rounded-lg p-4 border border-white/10">
            ...
            <select
              ...
              className="w-full px-4 py-3 bg-slate-700 border border-white/20 rounded-lg text-slate-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 text-sm font-medium"
            >
          ...
          <div className="bg-slate-800/50 rounded-lg p-4 border border-white/10">
            ...
            <div className="bg-slate-700 border border-white/20 rounded-lg p-3 max-h-52 overflow-y-auto">
              <label className="flex items-center gap-3 mb-3 sticky top-0 bg-slate-700 pb-2 border-b border-white/20">
                <input
                  type="checkbox"
                  checked={selectAllStates}
                  onChange={handleSelectAllStates}
                  className="w-4 h-4 rounded border-white/30 text-blue-500 focus:ring-2 focus:ring-blue-500 bg-slate-600"
                />
                <span className="text-sm text-slate-100 font-semibold">All States ({uniqueStates.length})</span>
              </label>
              <div className="space-y-2">
                {uniqueStates.map((state, i) => (
                  <label key={`${state || 'state'}__${i}`} className="flex items-center gap-3 py-2 px-2 rounded-md hover:bg-slate-600/60 cursor-pointer transition-colors duration-150">
                    <input
                      type="checkbox"
                      checked={selectAllStates || selectedStates.includes(state)}
                      onChange={() => handleStateToggle(state)}
                      className="w-4 h-4 rounded border-white/30 text-blue-500 focus:ring-2 focus:ring-blue-500 bg-slate-600 flex-shrink-0"
                    />
            ...
            <div className="bg-slate-700 border border-white/20 rounded-lg p-3 max-h-52 overflow-y-auto">
              ...
              <div className="space-y-2">
                {uniqueDistricts.map((district, i) => (
                  <label key={`${district || 'district'}__${i}`} className="flex items-center gap-3 py-2 px-2 rounded-md hover:bg-slate-600/60 cursor-pointer transition-colors duration-150">
                    <input
                      type="checkbox"
        ...
        <div className="text-sm text-slate-400 mb-3">
          Showing {chartData.length} {viewType === 'state' ? 'states' : viewType === 'district' ? 'districts' : 'entries'}
        </div>
      </div>
