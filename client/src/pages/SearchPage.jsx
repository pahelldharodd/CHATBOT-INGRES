...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    async function loadAll() {
            ...
            Papa.parse(path, {
              ...
              complete: (res) => {
                ...
                data.forEach((r) => {
                  ...
                  const key = `${y}||${primary}`;
                  if (!perYearAggregates[key])
                    perYearAggregates[key] = {
                      state,
                  ...
                  const gwe =
                    gweRaw != null && gweRaw !== ""
                      ? parseFloat(String(gweRaw).replace(/,/g, ""))
                      : NaN;
                  if (!Number.isNaN(gwe)) {
                  ...
                  const aer =
                    aerRaw != null && aerRaw !== ""
                      ? parseFloat(String(aerRaw).replace(/,/g, ""))
                      : NaN;
                  if (!Number.isNaN(aer)) {
                  ...
                  const ucaFr =
                    parseFloat(String(r["UCA_FR"] || 0).replace(/,/g, "")) || 0;
                  const ucaSl =
                    parseFloat(String(r["UCA_SL"] || 0).replace(/,/g, "")) || 0;
                  const cfaFr =
                    parseFloat(String(r["CFA_FR"] || 0).replace(/,/g, "")) || 0;
                  const cfaSl =
                    parseFloat(String(r["CFA_SL"] || 0).replace(/,/g, "")) || 0;
                  const tgahFr =
                    parseFloat(String(r["TGAH_FR"] || 0).replace(/,/g, "")) ||
                    0;
                  const tgahSl =
                    parseFloat(String(r["TGAH_SL"] || 0).replace(/,/g, "")) ||
                    0;
      ...
      const out = Object.values(perYearAggregates).map((s) => {
        const gwePct =
          s.aerSum > 0 && s.gweSum != null ? (s.gweSum / s.aerSum) * 100 : null;
      ...
      out.sort(
        (a, b) =>
          (a.primary || "").localeCompare(b.primary || "") || a.year - b.year
      );
      setRows(out);
      setErrors(missing);
      setLoading(false);
      ...
      Object.keys(stateDistricts).forEach((s) => {
        districtMap[s] = Array.from(stateDistricts[s]).sort();
      });
      ...
      setDistrictOptions([]);
      setYearOptions(
        Array.from(new Set(out.map((r) => r.year)))
          .sort((a, b) => a - b)
          .map(String)
      );
      if (typeof window !== "undefined") window.__stateDistricts = districtMap;
      if (typeof window !== "undefined")
        window.__stateForDistrict = stateForDistrict;
    }
  ...
  useEffect(() => {
    ...
    const map =
      typeof window !== "undefined" && window.__stateDistricts
        ? window.__stateDistricts
        : {};
